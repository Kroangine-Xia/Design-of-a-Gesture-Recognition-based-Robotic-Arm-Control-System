...
kf2.Q *= 0.01                               # 这里简单地假设过程噪声是恒定的，并且是相互独立的=
                                            # 设置测量噪声协方差矩阵，表示观测值的不确定性
kf2.R *= 0.3                                     # 这里简单地假设测量噪声是恒定的，并且是相互独立的
                                                        # 设置初始状态向量和初始协方差矩阵
...
def calculate_tt(point1, point2):
...
def hand_control(ser, a=900, b=900, c=900, d=900, e=900):
    ...
    ser.sendall(bytes([0xff, 0x02, 0x00, al, ah]))
    ...
    ser.sendall(bytes([0xff, 0x02, 0x01, bl, bh]))
    ...
    ser.sendall(bytes([0xff, 0x02, 0x02, cl, ch]))
    ...
    ser.sendall(bytes([0xff, 0x02, 0x03, dl, dh]))
    ...
    ser.sendall(bytes([0xff, 0x02, 0x04, el, eh]))
    ...
    with mp_holistic.Holistic(
        ...
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                cv2.putText(image, str(round(angle, 2)), tuple(
                    np.multiply(np.array([RHL.landmark[14].x, RHL.landmark[14].y]), [640, 480]).astype(int)),
                ...
                ah = int(hex_string[:2], 16)  # 将前两位十六进制转换为整数
                al = int(hex_string[2:], 16)  # 将后两位十六进制转换为整数
                s.sendall(bytes([0xff, 0x02, 0x0a, al, ah]))

                a = int(800 + (pose_angle[1]) / 70 * 1300)
                hex_string = hex(a)[2:].zfill(4)  # [2:] 去掉前缀"0x"，zfill(4) 用于补齐到4位
                # 提取高八位和低八位
                ah = int(hex_string[:2], 16)  # 将前两位十六进制转换为整数
                al = int(hex_string[2:], 16)  # 将后两位十六进制转换为整数
                s.sendall(bytes([0xff, 0x02, 0x07, al, ah]))
                ...
                ah = int(hex_string[:2], 16)  # 将前两位十六进制转换为整数
                al = int(hex_string[2:], 16)  # 将后两位十六进制转换为整数
                s.sendall(bytes([0xff, 0x02, 0x09, al, ah]))
            ...
            if results.right_hand_landmarks:
                ...
                hand_angle = kf.x                              # serdis.write('f1={:.2f}, f2={:.2f}\r\n'.format(hand_angle[0], kf.x[0]).encode('utf-8'))
                hand_angle = kf.x
                hand_control(s,
